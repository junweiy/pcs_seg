...
class DoubleConv(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=KERNEL_SIZE, order='crg', num_groups=32):
    ...
    def _add_conv(self, pos, in_channels, out_channels, kernel_size, order, num_groups):
        ...
        for i, char in enumerate(order):
            if char == 'r':
                ...
                self.add_module('relu' + str(pos), nn.ReLU(inplace=True))
            elif char == 'c':
                self.add_module('conv' + str(pos), nn.Conv3d(in_channels,
                                                        out_channels,
                                                        kernel_size,
                                                        padding=1))
                ...
                self.add_module('norm' + str(pos), groupnorm.GroupNorm3d(out_channels, num_groups=num_groups))
            elif char == 'b':
                is_before_conv = i < order.index('c')
                if is_before_conv:
                    # self.add_module(f'norm{pos}', nn.BatchNorm3d(in_channels))
                    self.add_module('norm' + str(pos), nn.BatchNorm3d(in_channels))
                else:
                    # self.add_module(f'norm{pos}', nn.BatchNorm3d(out_channels))
                    self.add_module('norm' + str(pos), nn.BatchNorm3d(out_channels))
            else:
                raise ValueError(
                    "Unsupported layer type. MUST be one of 'b', 'r', 'c'")
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=KERNEL_SIZE, is_max_pool=True,
        ...
        self.double_conv = DoubleConv(in_channels, out_channels,
                                      ...
                                      num_groups=num_groups)
        # self.double_conv = BasicBlock(in_channels, out_channels)
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, interpolate, kernel_size=KERNEL_SIZE,
