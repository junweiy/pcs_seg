...
class UNet3DTrainer:
    ...
    def __init__(self, model, optimizer, loss_criterion, accuracy_criterion,
        ...
        self.logger.info("Sending the model to " + str(device))
    ...
    def from_checkpoint(cls, checkpoint_path, model, optimizer, loss_criterion, accuracy_criterion, loaders,
        ...
        logger.info(
            "Checkpoint loaded. Epoch: %d. Best val accuracy: %f. Num_iterations: %d" % (state['epoch'], state['best_val_accuracy'], state['num_iterations']))
        ...
        for i, t in enumerate(train_loader):
            ...
            if hasattr(self.loss_criterion, 'ignore_index') and self.loss_criterion.ignore_index is not None:
                ...
                if len(unique_labels) == 1 and unique_labels.item() == self.loss_criterion.ignore_index:
                    self.logger.info('Skipping training batch %d (contains only ignore_index)...' % (i))
                    continue
            ...
            if type(accuracy) == float:
                train_accuracy.update(accuracy, input.size(0))
            else:
            ...
            if self.num_iterations % self.log_after_iters == 0:
                ...
                self.logger.info("epoch %d/%d Training stats. Loss: %f. Accuracy: %f" % (self.num_epoch, self.max_num_epochs - 1, train_losses.avg, train_accuracy.avg))
            ...
            if self.num_iterations % self.validate_after_iters == 0:
                ...
                if self._check_early_stopping(is_best):
                    self.logger.info('Validation accuracy did not improve for the last %d validation runs. Early stopping...' % (self.max_patience))
                    return True

            if self.max_num_iterations < self.num_iterations:
                self.logger.info(
                    'Maximum number of iterations %d exceeded. Finishing training...' % (self.max_num_iterations))
                return True
            ...
            with torch.no_grad():
                for i, t in enumerate(val_loader):
                    ...
                    if hasattr(self.loss_criterion, 'ignore_index') and self.loss_criterion.ignore_index is not None:
                        unique_labels = torch.unique(target)
                        if len(unique_labels) == 1 and unique_labels.item() == self.loss_criterion.ignore_index:
                            self.logger.info('Skipping validation batch %d (contains only ignore_index)...' % (i))
                            continue

                    output, loss, accuracy = self._forward_pass(input, target, weight)
                    val_losses.update(loss.item(), input.size(0))
                    if type(accuracy) == float:
                        val_accuracy.update(accuracy, input.size(0))
                    else:
                ...
                self.logger.info('Validation finished. Loss: %f. Accuracy: %f' % (val_losses.avg, val_accuracy.avg))
    ...
    def _adjust_learning_rate(self, decay_rate=0.75):
        ...
        self.logger.info('Changing learning rate from %f to %f' % (old_lr, new_lr))
    ...
    def _is_best_val_accuracy(self, val_accuracy):
        ...
        if is_best:
            self.logger.info(
                'Saving new best validation accuracy: %f' % val_accuracy)
    ...
    def _log_stats(self, phase, loss_avg, accuracy_avg):
        tag_value = {
            str(phase) + '_loss_avg': loss_avg,
            str(phase) + '_accuracy_avg': accuracy_avg
        }
