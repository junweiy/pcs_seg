...
class HDF5Dataset(Dataset):
    ...
    def __init__(self, raw_file_path, patch_shape, stride_shape, phase, label_file_path=None, raw_internal_path='raw',
                 label_internal_path='label', label_dtype=np.float32, transformer=transforms.BaseTransformer,
        ...
        if phase != 'test':
            ...
            # Tmp
            # self.label = np.where(self.label > 0.3, 1, 0)
            
            if self.label.ndim in [3, 4]:
...
def get_loaders(train_paths, raw_internal_path, label_internal_path, label_dtype, train_patch, train_stride,
                transformer, pixel_wise_weight=False, curriculum_learning=False,
                ignore_index=None):
    """
    ...
    for val_path in val_paths:
        val_dataset = HDF5Dataset(val_path, train_patch, train_stride,
                                  phase='val',
                                  label_dtype=label_dtype,
                                  raw_internal_path=raw_internal_path,
                                  label_internal_path=label_internal_path,
